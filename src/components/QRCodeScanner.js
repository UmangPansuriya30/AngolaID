...
const QRCodeScanner = ({ onQRCodeDetected, isActive = true }) => {
  ...
  }, [isActive, pulseAnim, scanLineAnim, cornerAnim]);

  // Simulate QR code detection
  useEffect(() => {
    if (onQRCodeDetected && isActive) {
      const timer = setTimeout(() => {
        ...
        if (isQRDetected) {
          ...
          onQRCodeDetected(mockQRData);
        }
      }, 2500); // Wait 2.5 seconds before detection

      return () => clearTimeout(timer);
    }
  }, [onQRCodeDetected, isActive]);
